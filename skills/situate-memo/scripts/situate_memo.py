#!/usr/bin/env python3
"""Situate (alias "ubermemo") memo runner — Python standard library only.

One entry point for the single-name research engine, usable by any agent that
holds nothing but a Doppler service-account token. Situate *situates* one
ticker: what you are exposed to (a factor basket), the current state, the odds
per horizon (historical base rates beside option-implied), what the business is
saying, cheap/rich zones, and a posture memo. It reports distributions and a
posture — never a point price target, and never buy/sell language.

    situate_memo.py health
    situate_memo.py build NVDA --format md
    situate_memo.py build NVDA --format pdf --out ./out
    situate_memo.py get NVDA --summary
    situate_memo.py chat NVDA --message "what would prove the read wrong?"
    situate_memo.py export NVDA --format json --out ./out
...
* ``--remote`` talks HTTP to the deployed Underlying API
  ...
  Nothing but the origin is needed; the server holds its own keys. Situate is a
  new engine and may not be deployed at a given origin yet — ``health`` reports
  ``situate_deployed``; when it is false, switch to ``--local``.
* ``--local`` runs the engine in-process in a checkout of
  ``underlying-analyzer-reboot`` via ``python -m app.situate.cli``, with the
...
#: The only variables the Situate engine (and this script's origin resolution)
...
#: the Doppler overlay is filtered down to exactly this set. Situate reuses the
#: Prism plumbing (``app/prism/data.py``, ``macro.py``, ``cache.py``,
#: ``store.py``), so the cache/store keys stay ``PRISM_*``.
ENGINE_SECRET_KEYS: frozenset[str] = frozenset(
    {
        ...
        "MASSIVE_STOCKS_PLAN",
        "MASSIVE_OPTIONS_PLAN",
        "MASSIVE_FINANCIALS_PLAN",
        "FRED_API_KEY",
        "FRED_BASE_URL",
        "SEC_USER_AGENT",
        "EXA_API_KEY",
        ...
        "SITUATE_TEXT_MODEL",
        "PRISM_TEXT_MODEL",
        # Packet persistence (reused Prism cache + store).
        "SUPABASE_URL",
...
USER_AGENT = "situate-memo-skill/1.0"


class SituateSkillError(RuntimeError):
...
def fetch_doppler_config(token: str, project: str, config: str) -> dict[str, str]:
    ...
    try:
        ...
        raise SituateSkillError(
            f"Doppler refused {project}/{config}: HTTP {exc.code}. "
            "Check that the service-account token can read that config."
        ) from None
    except urllib.error.URLError as exc:
        raise SituateSkillError(
            f"Doppler unreachable for {project}/{config}: {exc.reason}"
        ) from None
    except json.JSONDecodeError:
        raise SituateSkillError(
            f"Doppler returned a non-JSON body for {project}/{config}"
        ) from None
    if not isinstance(payload, dict):
        raise SituateSkillError(f"Doppler returned an unexpected shape for {project}/{config}")
...
def http(
    ...
    *,
    ...
    try:
        ...
        raise SituateSkillError(f"{method} {url} failed: {exc.reason}") from None
    except TimeoutError:
        raise SituateSkillError(f"{method} {url} timed out after {timeout}s") from None
...
def remote_json(origin: str, path: str, *, method: str = "GET", body: dict[str, Any] | None = None,
    ...
    if status >= 400:
        raise SituateSkillError(http_error_message(origin, path, status, payload))
    try:
        return json.loads(payload.decode("utf-8"))
    except json.JSONDecodeError:
        raise SituateSkillError(f"{origin}{path} returned a non-JSON body") from None
...
def resolve_origin(explicit: str | None, secrets: dict[str, str]) -> str:
    ...
    for candidate in (
        explicit,
        os.getenv("SITUATE_ORIGIN"),
        os.getenv("UNDERLYING_ORIGIN"),
        secrets.get("SITUATE_ORIGIN"),
...
def resolve_repo(explicit: str | None) -> Path:
    ...
    for candidate in candidates:
        if (candidate / "app" / "situate" / "cli.py").is_file():
...
def local_cli(
    ...
    *,
    ...
    """Run ``python -m app.situate.cli`` in the checkout with the secret overlay."""
    env = {**os.environ, **secrets}
    env.setdefault("PYTHONPATH", str(repo))
    command = [python_for(repo), "-m", "app.situate.cli", *args]
    note(f"note: running {' '.join(['python', '-m', 'app.situate.cli', *args])} in {repo}")
    try:
        ...
        raise SituateSkillError(f"cannot launch the local engine: {exc}") from None
    except subprocess.TimeoutExpired:
        raise SituateSkillError(f"the local Situate build exceeded {timeout}s") from None
...
def emit_local(
    ...
    if result.returncode == EXIT_ENGINE_MISSING:
        note(
            "error: the Situate engine is not importable in that checkout. Use "
...
def cmd_health(args: argparse.Namespace, _secrets: dict[str, str]) -> int:
    ...
    if args.local:
        ...
        engine = repo / "app" / "situate" / "engine.py"
        print(
            json.dumps(
                {
                    "mode": "local",
                    ...
                    "cli_present": (repo / "app" / "situate" / "cli.py").is_file(),
                },
                indent=2,
            )
        )
    ...
    status, _, _ = http(args.origin, "/api/situate/", timeout=HEALTH_TIMEOUT_S)
    print(
        json.dumps(
            {
                "mode": "remote",
                "origin": args.origin,
                "health": payload,
                "situate_route_status": status,
                "situate_deployed": status < 400,
            },
            indent=2,
        )
    )
...
def cmd_build(args: argparse.Namespace, secrets: dict[str, str]) -> int:
    ...
    if args.local:
        repo = resolve_repo(args.repo)
        cli_args = [ticker, "--export", args.format]
        if args.out:
    ...
    note(
        f"note: POST {args.origin}/api/situate {{'ticker': '{ticker}'}} — "
        "this can take 1-3 minutes"
    )
    packet = remote_json(args.origin, "/api/situate", method="POST", body=body,
...
def cmd_get(args: argparse.Namespace, secrets: dict[str, str]) -> int:
    ...
    if args.local:
        ...
        cli_args += ["--summary"] if args.summary else ["--export", "json"]
        if args.as_of:
            cli_args += ["--as-of", args.as_of]
    ...
    path = f"/api/situate/{urllib.parse.quote(ticker)}{suffix}{query}"
...
def cmd_chat(args: argparse.Namespace, secrets: dict[str, str]) -> int:
    ...
    if not message:
        raise SituateSkillError("--message is required and must not be empty")
    if args.local:
        repo = resolve_repo(args.repo)
        # The local CLI prints the reply text to stdout; the full JSON payload
        # (with conversation_id) is a --remote-only artifact.
        if args.json:
            note("note: --json is honoured only in --remote mode; local chat prints the reply text")
    ...
    result = remote_json(
        args.origin,
        f"/api/situate/{urllib.parse.quote(ticker)}/chat",
        method="POST",
        body=body,
    )
    if args.json:
...
def export_remote(args: argparse.Namespace, ticker: str) -> int:
    ...
    path = f"/api/situate/{urllib.parse.quote(ticker)}/export?{query}"
    status, payload, _ = http(args.origin, path, timeout=READ_TIMEOUT_S)
    if status >= 400:
        raise SituateSkillError(http_error_message(args.origin, path, status, payload))
    as_of = args.as_of or "latest"
    return write_or_print(payload, f"situate-{ticker}-{as_of}.{args.format}", args.format, args.out)
...
def cmd_export(args: argparse.Namespace, secrets: dict[str, str]) -> int:
    ...
    if args.local:
        repo = resolve_repo(args.repo)
        cli_args = [ticker, "--stored", "--export", args.format]
        if args.out:
            cli_args += ["--out", args.out]
        if args.as_of:
            cli_args += ["--as-of", args.as_of]
...
def add_common(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Underlying API origin for --remote. Falls back to SITUATE_ORIGIN, "
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="situate_memo.py",
        description=(
            "Build, read, chat with, and export Situate single-name research "
            "packets — exposure, state, odds per horizon, what options are "
            "pricing, the business, zones and a posture memo. Research only — "
            "distributions and a posture, never a point price target or buy/sell, "
    ...
    build.add_argument("--format", "-f", choices=FORMATS, default="md")
    ...
    chat.add_argument(
        "--json", action="store_true", help="Print the full chat payload (remote only)"
    )
    add_common(chat)

    export = sub.add_parser("export", help="Download a stored packet as md, json or pdf")
    export.add_argument("ticker")
    export.add_argument("--format", "-f", choices=FORMATS, default="md")
...
def main(argv: list[str] | None = None) -> int:
    ...
    except SituateSkillError as exc:
